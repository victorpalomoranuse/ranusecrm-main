...
router.put('/', async (req, res) => {
  try {
    const { bank_iban, bank_name, payment_methods, payment_notes, ai_budget_preferences, warranty_text, ai_reform_rules } = req.body;
    const updates = {};
    ...
    if (ai_budget_preferences !== undefined) updates.ai_budget_preferences = ai_budget_preferences?.trim() || null;
    if (warranty_text !== undefined) updates.warranty_text = warranty_text?.trim() || null;
    if (ai_reform_rules !== undefined) updates.ai_reform_rules = ai_reform_rules?.trim() || null;
