...
function TabAsignaciones({ projectId, categoryId, listadosIntro, listadosTitle, onIntroUpdated }) {
  ...
  const assign=async(product)=>{const isMat=product.category?.type==='material';const type=isMat?'material':'mobiliario';if(isAssigned(product.id,type)){toast.error(`"${product.name}" ya está asignado`);return;}const endpoint=isMat?`/client-projects/${projectId}/materials`:`/client-projects/${projectId}/equipment`;try{const payload=isMat?{name:product.name,category:product.category?.name,category_type:product.category?.type,catalog_product_id:product.id,image_url:product.photo_url||null}:{name:product.name,category:product.category?.name,category_type:product.category?.type,catalog_product_id:product.id,quantity:1,image_url:product.photo_url||null,extra_images:[],youtube_url:null};if(categoryId!=null)payload.category_id=categoryId;const{data}=await api.post(endpoint,payload);setAssigned(prev=>isMat?{...prev,materials:[data.material,...prev.materials]}:{...prev,equipment:[data.equipment,...prev.equipment]});toast.success(`"${product.name}" añadido`);}catch{toast.error('Error al asignar producto');}};
