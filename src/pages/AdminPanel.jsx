...
function ProductModal({ categories, allProducts, product, onClose, onSaved }) {
  ...
  const [lumens,setLumens]=useState(product?.lumens!=null?String(product.lumens):'');
  ...
  const [nivelUso,setNivelUso]=useState(product?.nivel_uso||'');
    ...
    <div className="ap-confirm-overlay" onClick={onClose}>
      <div className="ap-modal-inner" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="ap-modal-body">
          ...
          {(() => { const sameTypeOthers = categories.filter(c => c.id !== categoryId && c.type === categories.find(x => x.id === categoryId)?.type); if (!sameTypeOthers.length) return null; return (
            <div className="ap-field">
              <label>Categorías adicionales <span className="ap-optional">(opcional — para que este producto también aparezca en otras categorías)</span></label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {sameTypeOthers.map(c => { const checked = extraCategoryIds.includes(c.id); return (
                  <button key={c.id} type="button" className={`ap-catalog-pill${checked ? ' active' : ''}`} style={{ fontSize: '0.72rem' }} onClick={() => setExtraCategoryIds(prev => checked ? prev.filter(id => id !== c.id) : [...prev, c.id])}>{c.name}</button>
                ); })}
              </div>
            </div>
          ); })()}
          ...
          <div className="ap-field">
            <label>Nivel de uso <span className="ap-optional">(opcional — para que la IA asesore mejor según el perfil del cliente)</span></label>
            <div style={{display:'flex',gap:'0.4rem',flexWrap:'wrap'}}>
              {[{v:'',l:'Sin clasificar'},{v:'domestico',l:'Doméstico'},{v:'semi_profesional',l:'Semi profesional'},{v:'profesional',l:'Profesional'}].map(o=>(
                <button key={o.v} type="button" className={`ap-catalog-pill${nivelUso===o.v?' active':''}`} style={{fontSize:'0.72rem'}} onClick={()=>setNivelUso(o.v)}>{o.l}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{borderTop:'1px solid rgba(255,255,255,0.07)',marginTop:'0.5rem',paddingTop:'0.75rem'}}>
            <p style={{fontSize:'0.7rem',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.08em',color:'rgba(255,255,255,0.3)',marginBottom:'0.75rem'}}>Iluminación <span style={{textTransform:'none',fontWeight:400,color:'rgba(255,255,255,0.25)'}}>(opcional, solo si aplica)</span></p>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
              <div className="ap-field" style={{margin:0}}><label>Lúmenes</label><input className="ap-field-input" type="number" step="1" min="0" value={lumens} onChange={e=>setLumens(e.target.value)} placeholder="ej: 800"/></div>
              <div className="ap-field" style={{margin:0}}><label>Vatios (W)</label><input className="ap-field-input" type="number" step="0.1" min="0" value={watts} onChange={e=>setWatts(e.target.value)} placeholder="ej: 9.5"/></div>
            </div>
...
function SortableCategoryPill({ cat, active, count, onClick, onRename, onDelete }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: cat.id });
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1, zIndex: isDragging ? 10 : undefined };
  return (
    <button ref={setNodeRef} style={style} {...attributes} {...listeners} className={`ap-catalog-pill${active ? ' active' : ''}`} onClick={onClick}>
      {cat.name}
      <span className="ap-catalog-pill-count">{count}</span>
      <span className="ap-catalog-pill-del" role="button" onClick={e => { e.stopPropagation(); onRename(cat); }} title="Renombrar" style={{ marginLeft: 2 }}><Pencil size={10} /></span>
      <span className="ap-catalog-pill-del" role="button" onClick={e => { e.stopPropagation(); onDelete(cat.id, cat.name); }}>×</span>
    </button>
  );
}

function CatalogoLibrary() {
  const [tab,setTab]=useState('material'); const [activeCat,setActiveCat]=useState('all'); const [categories,setCategories]=useState([]); const [products,setProducts]=useState([]); const [types,setTypes]=useState([]); const [loading,setLoading]=useState(true); const [newCatName,setNewCatName]=useState(''); const [addingCat,setAddingCat]=useState(false); const [showCatInput,setShowCatInput]=useState(false); const [showProductModal,setShowProductModal]=useState(false); const [editProduct,setEditProduct]=useState(null); const [confirm,setConfirm]=useState(null); const [cart,setCart]=useState([]); const [showCart,setShowCart]=useState(false); const [showTypeInput,setShowTypeInput]=useState(false); const [newTypeName,setNewTypeName]=useState(''); const [addingType,setAddingType]=useState(false);
  const {toast}=useToast();
  const sensors=useSensors(useSensor(PointerSensor,{activationConstraint:{distance:5}}));
  const addToCart=(product)=>setCart(prev=>prev.some(p=>p.id===product.id)?prev:[...prev,product]);
  const removeFromCart=(id)=>setCart(prev=>prev.filter(p=>p.id!==id));
  const clearCart=()=>setCart([]);
  useEffect(()=>{Promise.all([api.get('/catalog/categories'),api.get('/catalog/products'),api.get('/catalog/types')]).then(([cats,prods,typs])=>{setCategories(cats.data.categories||[]);setProducts(prods.data.products||[]);const t=typs.data.types||[];setTypes(t);if(t.length&&!t.some(x=>x.slug===tab))setTab(t[0].slug);}).catch(()=>toast.error('Error al cargar catálogo')).finally(()=>setLoading(false));},[]);
  const handleTabChange=(t)=>{setTab(t);setActiveCat('all');};
  const handleAddType=async(e)=>{e.preventDefault();if(!newTypeName.trim())return;setAddingType(true);try{const{data}=await api.post('/catalog/types',{name:newTypeName.trim()});setTypes(prev=>[...prev,data.type]);setTab(data.type.slug);setActiveCat('all');setNewTypeName('');setShowTypeInput(false);}catch{toast.error('Error al crear tipo');}finally{setAddingType(false);}};
  const handleRenameType=async(type)=>{const name=window.prompt(`Nuevo nombre para "${type.name}"`,type.name);if(!name||!name.trim()||name.trim()===type.name)return;try{const{data}=await api.put(`/catalog/types/${type.id}`,{name:name.trim()});setTypes(prev=>prev.map(t=>t.id===type.id?data.type:t));toast.success('Tipo renombrado');}catch{toast.error('Error al renombrar tipo');}};
  const filteredCats=categories.filter(c=>c.type===tab);
  const inCategory=(p,catId)=>p.category_id===catId||p.extra_categories?.some(ec=>ec.id===catId);
  const visibleProducts=products.filter(p=>p.category?.type===tab&&(activeCat==='all'||inCategory(p,activeCat))).sort((a,b)=>{if(activeCat==='all'){const catA=a.category?.name||'';const catB=b.category?.name||'';if(catA!==catB)return catA.localeCompare(catB);}return(a.display_order??999999)-(b.display_order??999999);});
  const handleAddCategory=async(e)=>{e.preventDefault();if(!newCatName.trim())return;setAddingCat(true);try{const{data}=await api.post('/catalog/categories',{name:newCatName.trim(),type:tab});setCategories(prev=>[...prev,data.category]);setNewCatName('');setShowCatInput(false);}catch{toast.error('Error al crear categoría');}finally{setAddingCat(false);}};
      ...
      <div className="ap-catalog-type-tabs" style={{display:'flex',alignItems:'center',flexWrap:'wrap',gap:'0.3rem'}}>
        {types.map(t=>(
          <div key={t.id} style={{display:'inline-flex',alignItems:'center'}}>
            <button className={`ap-catalog-type-btn${tab===t.slug?' active':''}`} onClick={()=>handleTabChange(t.slug)}>{t.name}</button>
            {tab===t.slug&&<button type="button" className="ap-btn-icon" style={{marginLeft:2}} onClick={()=>handleRenameType(t)} title="Renombrar tipo"><Pencil size={11}/></button>}
          </div>
        ))}
        {showTypeInput?(
          <form onSubmit={handleAddType} style={{display:'inline-flex',gap:4,alignItems:'center'}}>
            <input className="ap-catalog-cat-input" value={newTypeName} onChange={e=>setNewTypeName(e.target.value)} placeholder="Nuevo tipo…" autoFocus/>
            <button type="submit" className="ap-btn ap-btn-primary ap-btn-sm" disabled={addingType||!newTypeName.trim()}>{addingType?'…':'Crear'}</button>
            <button type="button" className="ap-btn-icon" onClick={()=>{setShowTypeInput(false);setNewTypeName('');}}><X size={13}/></button>
          </form>
        ):(
          <button type="button" className="ap-catalog-type-btn" style={{opacity:0.6}} onClick={()=>setShowTypeInput(true)}><Plus size={13}/> Tipo</button>
        )}
      ...
      {tab!=='renders'&&<div className="ap-catalog-filters">
        <div className="ap-catalog-filter-pills">
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleCategoryDragEnd}>
            <SortableContext items={filteredCats.map(c=>c.id)} strategy={rectSortingStrategy}>
              {filteredCats.map(cat=>(<SortableCategoryPill key={cat.id} cat={cat} active={activeCat===cat.id} count={products.filter(p=>inCategory(p,cat.id)).length} onClick={()=>setActiveCat(cat.id)} onRename={handleRenameCategory} onDelete={handleDeleteCategory}/>))}
            </SortableContext>
          </DndContext>
...
function TabAsignaciones({ projectId, categoryId, listadosIntro, listadosTitle, onIntroUpdated }) {
  ...
  // El array "equipment" guarda TODO lo que no es material (mobiliario,
  // iluminación, cualquier tipo nuevo) — hay que filtrar además por
  // category_type para que cada pestaña (tab) muestre solo lo suyo.
  const assignedForTab=(t)=>{const source=t==='material'?assigned.materials:assigned.equipment;return visibleAssigned(source).filter(a=>(a.category_type||(t==='material'?'material':'mobiliario'))===t);};
  const isAssigned=(productId,type)=>assignedForTab(type).some(a=>a.catalog_product_id===productId);
  const handleAssignedDragEnd=async(event)=>{const{active,over}=event;if(!active||!over||active.id===over.id)return;const isMat=tab==='material';const list=assignedForTab(tab);const oldIndex=list.findIndex(a=>a.id===active.id);const newIndex=list.findIndex(a=>a.id===over.id);const newList=arrayMove(list,oldIndex,newIndex);setAssigned(prev=>{const key=isMat?'materials':'equipment';const others=prev[key].filter(x=>!newList.some(n=>n.id===x.id));return{...prev,[key]:[...others,...newList]};});const endpoint=isMat?`/client-projects/${projectId}/materials/reorder`:`/client-projects/${projectId}/equipment/reorder`;try{await api.put(endpoint,{ids:newList.map(a=>a.id)});}catch{toast.error('Error al guardar el orden');}};
  const assign=async(product)=>{const isMat=product.category?.type==='material';const type=isMat?'material':(product.category?.type||'mobiliario');if(isAssigned(product.id,type)){toast.error(`"${product.name}" ya está asignado`);return;}const endpoint=isMat?`/client-projects/${projectId}/materials`:`/client-projects/${projectId}/equipment`;try{const lightingSpecs={lumens:product.lumens||null,watts:product.watts||null,color_temperature:product.color_temperature||null,color:product.color||null};const payload=isMat?{name:product.name,category:product.category?.name,category_type:product.category?.type,catalog_product_id:product.id,image_url:product.photo_url||null,...lightingSpecs}:{name:product.name,category:product.category?.name,category_type:product.category?.type,catalog_product_id:product.id,quantity:1,image_url:product.photo_url||null,extra_images:[],youtube_url:null,...lightingSpecs};if(categoryId!=null)payload.category_id=categoryId;const{data}=await api.post(endpoint,payload);setAssigned(prev=>isMat?{...prev,materials:[data.material,...prev.materials]}:{...prev,equipment:[data.equipment,...prev.equipment]});toast.success(`"${product.name}" añadido`);}catch{toast.error('Error al asignar producto');}};
  ...
  if(loading) return <div className="ap-loading">Cargando…</div>;
  const filteredCats=categories.filter(c=>c.type===tab);
  const assignedList=assignedForTab(tab);
  return (
    <div className="ap-tab-content">
      {categoryId==null&&(
...
function TabCategorias({ projectId }) {
  ...
  const handleLabelChange = async (field, value) => {
    if (!active) return;
    try {
      const { data } = await api.put(`/categories/${active.id}`, { [field]: value });
      setCategories(prev => prev.map(c => c.id === active.id ? { ...c, [field]: data.category[field] } : c));
    } catch {}
  };
    ...
    <div className="ap-tab-content">
      ...
      {!active ? (
        ...
        <>
          ...
          <p className="ap-tab-desc" style={{ marginTop: '1.5rem' }}>Catálogo de materiales y mobiliario de esta categoría. El título de cada listado es el que verá el cliente — ponle el nombre que quieras.</p>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            <div className="ap-field" style={{ flex: 1, minWidth: 200 }}>
              <label>Título del listado de materiales</label>
              <input className="ap-field-input" defaultValue={active.materials_label || ''} key={`ml-${active.id}`} placeholder="Materiales" onBlur={e => handleLabelChange('materials_label', e.target.value)} />
            </div>
            <div className="ap-field" style={{ flex: 1, minWidth: 200 }}>
              <label>Título del listado de mobiliario/equipamiento</label>
              <input className="ap-field-input" defaultValue={active.equipment_label || ''} key={`el-${active.id}`} placeholder="Mobiliario y equipamiento" onBlur={e => handleLabelChange('equipment_label', e.target.value)} />
            </div>
          </div>
          <TabAsignaciones projectId={projectId} categoryId={active.id} />
        </>
      )}
...
function SectionTrabajos() {
  ...
  const loadProjects=async()=>{try{const{data}=await api.get('/client-projects/public/portfolio');const list=data.projects||[];setProjects(list);setSavedProjects(list);}catch{}finally{setLoading(false);}};
  ...
  const handleSaveOrder=async()=>{setSaving(true);try{await api.put('/client-projects/portfolio/reorder',{items:projects.map(p=>({id:p.id,type:p.type}))});setSavedProjects(projects);toast.success('Orden guardado correctamente');}catch{toast.error('Error al guardar el orden');}finally{setSaving(false);}};
  ...
  return (
    <div className="ap-section">
      ...
      {loading?<div className="ap-loading">Cargando proyectos...</div>:projects.length===0?<div className="ap-empty"><p>No hay proyectos todavía.</p></div>:(
        ...
        <div className="ap-projects-grid">{projects.map((p,i)=>{const validCover=p.cover_url&&!p.cover_url.startsWith('blob:')&&!brokenCovers[p.id];const fallback=p.images?.find(u=>u&&!u.startsWith('blob:'));const src=validCover?p.cover_url:fallback;const isLinked=p.type==='linked';return(<div key={p.id} className={`ap-project-card${i<3?' is-featured':''}`}><div className="ap-project-img">{src?<img src={src} alt={p.title} onError={()=>setBrokenCovers(prev=>({...prev,[p.id]:true}))}/>:<div className="ap-project-no-img">Sin imagen</div>}{p.images?.length>0&&<span className="ap-project-count">{p.images.length} fotos</span>}{isLinked&&<span className="ap-featured-badge" style={{background:'rgba(190,176,162,0.25)'}}>Proyecto conectado</span>}{i<3&&<span className="ap-featured-badge">En inicio</span>}</div><div className="ap-project-info"><h3>{p.title}</h3>{p.description&&<p>{p.description}</p>}<span className="ap-project-slug">/{p.slug}</span></div><div className="ap-project-actions"><div className="ap-order-btns"><button className="ap-btn ap-btn-ghost ap-btn-sm" onClick={()=>handleMove(i,-1)} disabled={i===0}><ChevronUp size={14}/></button><button className="ap-btn ap-btn-ghost ap-btn-sm" onClick={()=>handleMove(i,1)} disabled={i===projects.length-1}><ChevronDown size={14}/></button></div>{isLinked?<span style={{fontSize:'0.72rem',color:'rgba(255,255,255,0.35)'}}>Edítalo desde su proyecto → Trabajos web</span>:(<><button className="ap-btn ap-btn-ghost ap-btn-sm" onClick={()=>setModal(p)}><Pencil size={13}/> Editar</button><button className="ap-btn ap-btn-danger ap-btn-sm" onClick={()=>setConfirmId(p.id)}><Trash2 size={13}/></button></>)}</div></div>);})}</div></>
      )}
...
function SectionProyectos() {
  ...
  useEffect(()=>{loadProjects();},[]);
  const abrirRelacion=async(p)=>{
    try{
      const{data}=await api.get('/ventas',{params:{client_project_id:p.id}});
      const ventas=data.ventas||[];
      if(ventas.length===0)toast.error('Este proyecto todavía no tiene ninguna venta enlazada');
      else if(ventas.length===1)setVerProyectoVenta(ventas[0].id);
      else setElegirVentas({proyecto:p,ventas});
    }catch{toast.error('Error al buscar las ventas del proyecto');}
  };
  ...
  return (
    <div className="ap-section">
      ...
      {elegirVentas&&(
        <div className="ap-modal-overlay" onClick={()=>setElegirVentas(null)}>
          <div className="ap-modal" onClick={e=>e.stopPropagation()} style={{maxWidth:420}}>
            <div className="ap-modal-head"><h2>Elige una venta</h2><button className="ap-modal-close" onClick={()=>setElegirVentas(null)}><X size={16}/></button></div>
            <div style={{padding:'0 1.25rem 1.25rem',display:'flex',flexDirection:'column',gap:6}}>
              <p style={{fontSize:12,color:'rgba(255,255,255,0.5)',margin:0}}>{elegirVentas.proyecto.project_name} tiene {elegirVentas.ventas.length} ventas enlazadas:</p>
              {elegirVentas.ventas.map(v=>(
                <button key={v.id} className="ap-btn ap-btn-ghost ap-btn-sm" style={{justifyContent:'flex-start'}} onClick={()=>{setVerProyectoVenta(v.id);setElegirVentas(null);}}>{v.nombre}</button>
              ))}
            </div>
          </div>
        </div>
...
const ETIQUETAS_PERMISOS = {
  ...
  catalogo: 'Catálogo', referencias: 'Referencias', recursos: 'Recursos', contactos: 'Proveedores',
...
function SectionClientes() {
  ...
  const handleCreate=async(e)=>{e.preventDefault();setFormError('');setCreating(true);try{const{data}=await api.post('/users/cliente',{email,password,name:nombreCliente});setClients(prev=>[data.user,...prev]);setNombreCliente('');setEmail('');setPassword('');setShowModal(false);toast.success('Cliente creado');}catch(err){setFormError(err.response?.data?.error||'Error al crear el cliente');}finally{setCreating(false);}};
...
function SectionContactos() {
  ...
  return (
    <div className="ap-section">
      ...
      {loading?<div className="ap-loading">Cargando…</div>:contacts.length===0?<div className="ap-empty"><p>No hay proveedores todavía.</p></div>:(<div className="ap-contacts-list">{visible.map(c=>(<div key={c.id} className="ap-contact-row"><div className="ap-contact-avatar">{c.name.charAt(0).toUpperCase()}</div><div className="ap-contact-info"><div className="ap-contact-header"><span className="ap-contact-name">{c.name}</span>{c.sector&&<span className="ap-contact-sector">{c.sector}</span>}</div><div className="ap-contact-details">{c.phone&&(<a href={`tel:${c.phone}`} className="ap-contact-detail"><Phone size={11}/> {c.phone}</a>)}{c.email&&(<a href={`mailto:${c.email}`} className="ap-contact-detail">{c.email}</a>)}{c.company_url&&(<a href={c.company_url} target="_blank" rel="noopener noreferrer" className="ap-contact-detail ap-contact-web"><ExternalLink size={11}/> Web</a>)}</div>{c.description&&<p className="ap-contact-desc">{c.description}</p>}</div><div className="ap-contact-actions"><button className="ap-btn ap-btn-ghost ap-btn-sm" onClick={()=>setModal(c)}><Pencil size={12}/> Editar</button><button className="ap-btn ap-btn-danger ap-btn-sm" onClick={()=>handleDelete(c.id)}><Trash2 size={12}/></button></div></div>))}</div>)}
    </div>
  );
}
...
const NAV_ITEMS = [
  ...
  { id:'contactos', label:'Proveedores', Icon:Phone, permission:'contactos' },
