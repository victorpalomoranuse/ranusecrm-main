...
function PagoRow({ pago, onDelete, onSavedProveedor, readOnly }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12.5, padding: '6px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
      ...
      {readOnly ? (
        <span style={{ color: 'rgba(255,255,255,0.5)', minWidth: 140 }}>{proveedor || 'Sin proveedor'}</span>
      ) : (
        <input
      ...
      {!readOnly && <button className="ap-btn-icon" onClick={() => onDelete(pago.id)}><Trash2 size={12} /></button>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
        <div><div style={{ fontSize: 10, color: '#8bae8f', textTransform: 'uppercase' }}>Cobrado del cliente</div><strong style={{ fontSize: 15, color: '#8bae8f' }}>{fmt(rel.ingresoTotal)}</strong></div>
        ...
        <div><div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase' }}>Relación real (gastos)</div><strong style={{ fontSize: 15, color: '#ae6b6b' }}>{fmt(rel.pagadoTotal)}</strong></div>
        ...
        <div className="ro-tabla" style={{ marginBottom: 24 }}>
          ...
          {rel.proveedores.map(p => (
            <div key={p.proveedor} className="ro-row ro-row--5">
              ...
              <span>{puedeGestionar ? <PrevistoInput prov={p} onSaved={handleSavePrevisto} /> : <span style={{ color: p.previstoEditado ? '#a78bfa' : undefined }}>{fmt(p.previsto)}</span>}</span>
      ...
      <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.35)', marginBottom: 8 }}>Relación real — gastos ({rel.pagos.length})</p>
      {rel.pagos.length === 0 ? <p className="ap-empty-sm">Todavía no hay gastos registrados para esta obra.</p> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 24 }}>
          {rel.pagos.map(p => (
            <PagoRow key={p.id} pago={p} onDelete={handleDeletePago} onSavedProveedor={cargar} readOnly={!puedeGestionar} />
          ))}
        </div>
      )}

      <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.35)', marginBottom: 8 }}>Ingresos cobrados al cliente ({rel.ingresos.length})</p>
      {rel.ingresos.length === 0 ? <p className="ap-empty-sm">Todavía no hay ingresos registrados para esta obra.</p> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {rel.ingresos.map(i => (
            <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12.5, padding: '6px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
              <span style={{ flex: 1 }}>{i.concepto}</span>
              {i.categoria && <span style={{ color: 'rgba(255,255,255,0.4)' }}>{i.categoria}</span>}
              <span style={{ color: 'rgba(255,255,255,0.4)' }}>{i.fecha?.slice(0, 10)}</span>
              <span style={{ color: '#8bae8f', minWidth: 70, textAlign: 'right' }}>+{fmt(i.monto)}</span>
            </div>
          ))}
        </div>
      )}
